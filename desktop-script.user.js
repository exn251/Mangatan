...
(function() {
    ...
    let settings = {
        ...
        debugMode: false, textOrientation: 'smart', interactionMode: 'hover', dimmedOpacity: 0.3, fontMultiplierHorizontal: 1.0,
        fontMultiplierVertical: 1.0, boundingBoxAdjustment: 5, focusScaleMultiplier: 1.1, soloHoverMode: true,
        deleteModifierKey: 'Alt', mergeModifierKey: 'Control', addSpaceOnMerge: false, colorTheme: 'grey', brightnessMode: 'light',
...
function calculateAndApplyStylesForSingleBox(box, imgRect) {
    ...
    // Use adjusted dimensions for fitting calculations with 90% target usage
    ...
    const findBestFitSize = (isVerticalSearch) => {
        ...
        if (isMerged) {
            // For merged boxes: preserve original line breaks, don't allow additional wrapping
            measurementSpan.style.whiteSpace = 'pre'; // Preserve line breaks exactly
            // Convert \u200B to actual line breaks for measurement
            measurementSpan.innerHTML = text.replace(/\u200B/g, "<br>");
        } else {
            // For non-merged boxes: use normal wrapping behavior
        ...
        while (low <= high) {
            ...
            // Check if the measured dimensions fit within 90% of available space
    ...
    // Special handling for merged boxes: ensure line breaks are preserved
    const findBestFitSizeForMerged = (isVerticalSearch) => {
        ...
        while (low <= high) {
            ...
            // For merged boxes, we need to ensure each line fits properly
            const fitsWidth = measuredWidth <= availableWidth;
            const fitsHeight = measuredHeight <= availableHeight;

            // Additional check: make sure individual lines aren't too long
            let linesFit = true;
            if (fitsWidth && fitsHeight) {
                // Check if any individual line would overflow (approximate by checking if text is clipping)
                ...
                // If any line would be significantly wider than available space, reduce font size
    ...
    if (isMerged) {
        // For merged boxes: preserve original line breaks exactly
        box.style.whiteSpace = 'pre'; // Changed from 'normal' to 'pre'
        box.style.textAlign = 'start';
        // Ensure line breaks are rendered properly
        box.innerHTML = text.replace(/\u200B/g, "<br>");
    } else {
        box.style.whiteSpace = 'nowrap';
        box.textContent = text; // Clear any previous HTML content
    }
}
    ...
    document.querySelectorAll('.gemini-ocr-text-box.selected-for-merge').forEach(box => {
        box.classList.remove('selected-for-merge');
    });
...
function enterEditMode(textBox, sourceImage) {
    ...
    const saveEdit = () => {
        // FIX: Guard clause against double-firing (Enter followed immediately by Blur)
        // If originalText is null, exitEditMode has already run.
    ...
    // Event listeners
    // Note: We keep 'once: true' but the guard clause in saveEdit handles the edge case
    textBox.addEventListener('blur', saveEdit, { once: true });

    textBox.addEventListener('keydown', (e) => {
        if (e.key === 'Enter' && !e.shiftKey) {
            e.preventDefault();
            // Manually remove the blur listener so it doesn't fire after we force save
            // (Though the guard clause above covers this, removing the listener is cleaner)
...
function saveTextChanges(textBox, sourceImage, newText) {
    ...
    function displayOcrResults(targetImg) {
        ...
        data.forEach((item, index) => {
            ...
            if (item.isMerged) {
                ocrBox.style.whiteSpace = 'pre-line';
...
function showImageSelectionDialog(validImages) {
    return new Promise((resolve) => {
        ...
        // Create wrapper to center the scrolling container
        ...
        // Create container for image previews - MODIFIED FOR CENTERED HORIZONTAL SCROLLING
        ...
        Object.assign(container.style, {
            ...
            margin: '0 auto' // Center the container within wrapper
        });
        ...
        validImages.forEach((item, index) => {
            ...
            // QUALITY IMPROVEMENT: Use higher resolution but limit height to 480px max
            const maxThumbWidth = 400;
            const maxThumbHeight = 480; // Limited to 480px max height

            // Calculate scale to fit height within max dimensions while maintaining aspect ratio (only height constrained)
            const scale = Math.min(
                maxThumbHeight / item.image.naturalHeight,
                1  // Don't upscale
            );

            // QUALITY IMPROVEMENT: Use 2x resolution for crisp display on high DPI screens
            ...
            // QUALITY IMPROVEMENT: Enable image smoothing with high quality
            ...
            // QUALITY IMPROVEMENT: Set CSS dimensions to half of canvas for crisp rendering
        ...
        // Add the container to the wrapper, and wrapper to overlay
    ...
    return new Promise((resolve) => {
        ...
        function createButton(text, bgColor, leftOffset, transformX) {
            ...
            Object.assign(button.style, {
                ...
                padding: '15px 25px', // Larger for mobile
                fontSize: '18px', // Larger for mobile
                background: bgColor,
                color: '#fff',
                border: 'none',
                borderRadius: '8px', // Larger radius for mobile
                cursor: 'pointer',
                zIndex: '10000000',
...
async function handleConfirm() {
    ...
    try {
        ...
        cleanup();
        resolve(success); // This resolves the main exportImageToAnki promise
    } catch (error) {
    ...
    return new Promise((resolve) => {
        ...
        reader.onload = async () => {
            ...
            try {
                ...
                // 3. Update note USING picture API (no renaming!)
                await ankiConnectRequest("updateNoteFields", {
                    note: {
                        id: lastNoteId,
                        fields: {
                            [settings.ankiImageField]: "" // optional: clear or leave empty
                        },
                        picture: {
                            filename: filename,
                            data: base64Data,
                ...
                resolve(true); // SUCCESS - make sure this gets called

            } catch (error) {
                console.error("Anki error:", error);
                alert("Error sending to Anki: " + error.message);
                resolve(false); // FAILURE - make sure this gets called
            }
        };
